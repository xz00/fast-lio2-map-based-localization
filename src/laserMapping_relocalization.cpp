...
void initialpose_callback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& pose_msg){
    ...
    publishCloud(&pubBeforeRegistCloud,after_guess,ros::Time().now(),"camera_init");//发布局部地图
}
...
int main(int argc, char **argv)
{
    ...
    nh.param<string>("globalmap_dir", globalmap_dir, "/Downloads/LOAM/");
    ...
    if(matching_ptr_->HasNewGlobalMap()){
        ...
        publishCloud(&pubGobalMap,global_map_ptr,ros::Time::now(),"camera_init");
    }
    ...
    while (ros::ok())
    {
        ...
        if (sync_packages(Measures)) //把一次的IMU和LIDAR数据打包到Measures
        {
            ...
            while(!inited_pose){
                ...
                if(init_flag == InitializedFlag::NonInitialized){
                    ...
                    if(matching_ptr_->InitFromGuess(initial_guess,scan_IMUframe,result) != true){//初始化失败
                        ...
                        publishCloud(&pubRegistCloud,matching_ptr_->GetCurrentScan(),ros::Time::now(),"camera_init");//发布匹配后的点云位姿
            ...
            if(matching_ptr_->HasInited()){
                ...
                if(relocal_scan_en){
                    publishCloud(&pubRegistCloud,matching_ptr_->GetCurrentScan(),ros::Time().fromSec(lidar_end_time),"camera_init");//发布匹配后的点云位姿
                }
                if(localmap_en && matching_ptr_->HasNewLocalMap()){
                    publishCloud(&pub_local_map,matching_ptr_->GetLocalMap(),ros::Time().fromSec(lidar_end_time),"camera_init");//发布局部地图
                }
            }
